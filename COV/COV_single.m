function SMresult = COV_single(imgfile)

% salSize   = options.size;
% ratio     = options.quantile;
% modeltype = options.modeltype;
salSize = 512;                     % size of rescaled image
ratio = 1/10;                % parameter specifying the most similar regions in the neighborhood
options.centerBias = 1;                 % 1 for center bias and 0 for no center bias
modeltype = 'CovariancesOnly';
% read image
% disp('Reading image.');
% imgfile='fish.png';


g = double(imread(imgfile));
...
SMresult = mat2gray(salmapSmooth);